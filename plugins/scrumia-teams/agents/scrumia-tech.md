...
4. **Coverage** — is every acceptance criterion the change is answerable for backed by something that could have failed? A test that cannot fail tests nothing — and a section no concrete case could contradict covers nothing either. The form follows the criterion's own subject, not the deliverable's: a test where the subject is executable behaviour, the section that satisfies it where the subject is prose and there is nothing to execute ([`features/business/dev-flow/business.md`](../../../features/business/dev-flow/business.md) § *Covering a criterion*). Do not demand a test for a criterion with nothing to execute — and do not accept an absent harness or a prose half of the same ticket as an excuse for one that does.
