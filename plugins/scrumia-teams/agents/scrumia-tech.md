...
4. **Coverage** — is every acceptance criterion the change is answerable for backed by a **test**, in the form its own subject calls for? Test is the umbrella: a unit or integration test, an automated check over the project's own prose, an audit by the role that owns the judgement, a checklist walked case by case. The form follows the criterion's subject, not the deliverable's ([`features/business/dev-flow/business.md`](../../../features/business/dev-flow/business.md) § *Covering a criterion*, which maps subject to form with a worked example each). A test that cannot fail tests nothing — and **a section reference covers nothing**: it says where the thing lives, and the same pointer gets written whether the section says what the criterion demands or its opposite. Two symmetrical errors to avoid: demanding a unit test of a criterion whose subject is a rule's wording, and accepting an absent harness, an approximate check, a blanket gate 2 verdict that never names the criterion, or the prose half of a mixed ticket as an excuse for a criterion whose subject is behaviour or a decidable property of the text.
