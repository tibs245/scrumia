...
**Entries carrying a `from:` key are the enablement-side link, not a transitional artifact.** `settings.team.roles` enables the roles `scrumia-extends convene` enumerates; `from:` is the project's side of the cross-check, naming the `<source>:<module>` key the role's `extends:` set must contain ([ADR-0014](https://github.com/tibs245/scrumia/blob/main/docs/adr/0014-roles-ship-with-their-capability.md), `features/business/agent-team/` § *Roles*). Another module can fill a slot and ship the standing role that guards it — `scrumia-design` does this with `designer`. Its definition lives in that module, not in `agents/`, and removing the entry here silently unplugs it. Leave the entry alone, and send any change to it back to its own module's setup skill. Reachability is checked elsewhere: `compose-status.sh` (which closes `scrumia-init`) reads `claude plugin list --json` and reports any declared-but-not-installed module on stderr; the standing-role version of that check is `features/business/agent-team/`'s AC-13.
