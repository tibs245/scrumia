...
- `scrumia-place` — one tree from something just learned to exactly one destination: a
  ...
  intercepts nothing.
- A `PostToolUse` hook on `Write` and `Edit` — a write under `.claude/agent-memory/` or a
  `.claude/projects/<project>/memory/` directory answers with a reminder to run
  `scrumia-place` on the entry, naming it. Installed with the module, it runs after the
  write and refuses nothing; it stays silent on an index file, without `jq`, and outside a
...
- `scrumia-author` — the pass from a need to a module `scrumia-module check` accepts on
  ...
  included, comes out byte-identical. Either way it reports the type and the scope the
  commit will carry, and no bump level.
