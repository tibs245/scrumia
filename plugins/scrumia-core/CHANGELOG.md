...
- `scrumia-module` — states whether one module meets the anatomy standard, from that
  ...
  exit status separates five states a boolean collapses (`0` clean, `1` the tool failed,
  `2` bad usage, `3` findings, `4` not a module), so read `state`, never the length of
  `findings`: a non-zero exit is not a finding.
