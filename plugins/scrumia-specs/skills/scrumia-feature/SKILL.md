...
| `business.md` | **mandatory** — the rules, personas, value, journey-as-intent | reference to the parent | reference to the parent |
...
Three existence categories, declared in `references/catalog.md`: **mandatory in every
feature** (`index.md`, `qa.md`, `CHANGELOG.md`), **mandatory per stratum**
(`business.md` at the Business stratum), **content-tested** (everything else).
...
**This block does not declare the mandatory set, and a consumer must not infer one from it.** A key names a file, it does not require it — a module that carries a changelog without mandating it still needs a `changelog:` key, or consumers have no name to use. The three files mandatory in *every* feature are the three named outside `catalog` (`index.md`, `qa.md`, `CHANGELOG.md`); `business.md`, mandatory at the Business stratum only, sits inside `catalog` because a key's position says nothing about status. Both facts are `references/catalog.md`'s to declare, where this module declares its existence categories — not the contract's.
