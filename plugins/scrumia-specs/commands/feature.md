...
Beyond `index.md`, `qa.md` and `CHANGELOG.md`, which this module requires of every feature, write the file the content calls for and no more. An absent optional file is information — it says the question does not arise — while an empty one says nobody looked.

A spec holds only its current version. History lives in git and in the tickets.
