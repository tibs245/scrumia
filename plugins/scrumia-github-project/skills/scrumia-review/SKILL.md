...
**The gap becomes a reportable scoping signal only when [ADR-0015](https://github.com/tibs245/scrumia/blob/main/docs/adr/0015-scope-measures-reach.md)'s own questions — how many apps, does a rule consumed beyond one feature or app change, does an interface contract change — would have answered higher than the label carries.** The two grids disagree routinely and correctly since 0015 decoupled them, so a disagreement on its own reports nothing; calling every one of them a mislabel fires on a whole class of correctly-labelled ticket.

**Where it is owed, record it on the issue, addressed to the manager** — the role that set the label and routes on it ([`features/business/dev-flow/business.md`](https://github.com/tibs245/scrumia/blob/main/features/business/dev-flow/business.md) § *Gate 2's scoping signal*), before Step 5 rather than inside it:

```bash
gh issue comment <issue> --body "Scoping signal: label <label> — review routed <roles> — axis question <1|2|3> answers yes on <what>"
```

**`<issue>` is the ticket's number, not `<n>`.** You were invoked on a pull request; the ticket is the one its body closes, which the `gh pr view` above already fetched — read `Closes #<issue>` out of that body. The two are not interchangeable and substituting the PR number fails silently rather than loudly: GitHub numbers pull requests and issues in one sequence and treats a pull request as an issue, so `gh issue comment <pr>` posts onto the pull request and exits zero. That leaves an echo with no record behind it — exactly the state [`features/business/github-tracking/`](https://github.com/tibs245/scrumia/blob/main/features/business/github-tracking/qa.md)'s AC-15 declares a failure. If the body carries no closing keyword, say so in the synthesis and record nothing rather than guessing a number.

Step 5's synthesis then echoes it, and that echo is a copy. The record has to outlive the review: Step 6 publishes with `gh pr comment`, and the retrospective's trigger counts these gaps ([`features/business/ceremonies/`](https://github.com/tibs245/scrumia/blob/main/features/business/ceremonies/business.md)) by reading records against tickets, not pull-request threads. Same record/echo split `scrumia-ticket` applies at its own Step 6 — the two implementations of gate 2 must not disagree about this any more than about who owed the review.

*Every reference in this section is provenance, in `module-anatomy`'s BR-12 sense: the condition, the recipient, the venue and the record/echo split are all stated above, and the links name where each is owned. Read the way this step runs off this file; open the specs to argue with the rule, not to apply it.*
