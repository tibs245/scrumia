...
- A criterion this PR is answerable for with nothing covering it — the ticket's own, plus those it adds to or amends in the file named by `acceptance_file`. A criterion already standing in that file and untouched by this PR is **not** in that set, and raising it here false-blocks every PR that amends one criterion in a file holding ten. What counts as covering one is stated in [`features/business/dev-flow/business.md`](../../../../features/business/dev-flow/business.md) § *Covering a criterion*: a **test**, in whichever form the criterion's own subject calls for — a unit or integration test, an automated check over the project's prose, an audit by the role that owns the judgement, or a checklist walked case by case. Read it before raising this one. Two ways to get it wrong, in opposite directions. **Over-demanding** — raising this finding because no unit test covers a criterion whose subject is a rule's wording; its form is the audit you are performing, and the verdict you return on that criterion is what covers it. **Under-demanding** — letting a criterion pass on a **section reference**, which states a location and cannot come out otherwise; on a blanket "approved at gate 2" that never names the criterion; on an absent harness; on an approximate check that would pass a case the criterion forbids; or because the same ticket also ships prose. Each of those is this defect, and it is raised. So is a criterion no concrete case could contradict, whichever section the PR points at.
