...
1. **Resolve the app by path** — match the file against `apps[].path` in `.scrumia/config.yaml`. No match, no module: follow the conventions of the neighboring code. That's normal behavior, not a gap. It is also the answer where a file matches nothing because the **product itself is not one of the apps** — a project whose deliverable is its own specs, prose or plugins. Where the composition names such paths, `CLAUDE.md` says so and you read it there; where it names none, an unmatched path under the product's own tree is still not a hole in the table. Either way, "neighboring code" then means the neighboring **prose**: match its voice, its structure and its vocabulary as strictly as you would match a language's.
...
Whether an implementation module is plugged in or not, cover every acceptance criterion this ticket is answerable for with **a test** — an act of checking that could have come out the other way. A unit test is one form of test, an integration test another. So are an automated check over the project's own prose, an audit by a role at Step 6, and a checklist walked case by case. **The form follows the criterion's own subject, not the deliverable's overall nature.** The authoritative statement — the six forms, what makes each one fail, and a worked example of each — is [`features/business/dev-flow/business.md`](../../../../features/business/dev-flow/business.md) § *Covering a criterion*. Its mapping, read from here:
...
**Ask the role about the criteria, not only about the diff.** Step 7's mapping has to say what the role said about *each* criterion whose subject is a judgement — an audit covers the criteria its verdict addresses and no others, so one overall verdict leaves the rest uncovered ([`features/business/dev-flow/business.md`](../../../../features/business/dev-flow/business.md) § *A section reference is not a form of coverage*). List those criteria in the prompt you hand the role and ask for one line per criterion alongside its overall verdict.
...
- **The criterion-by-criterion mapping, against the issue's own criteria** — one line per identifier as the *issue* numbers them, cited bare (`AC-3`). Each line carries **two** things: where what satisfies the criterion lives, and **which form checked it, with its outcome** — the test that runs it, the check in the harness, the role that audited it at Step 6 and what that role said about *this* criterion, or the checklist case you walked and what it returned. A location on its own is not coverage; a form on its own leaves the reviewer hunting. A blanket "approved at gate 2" copied onto ten lines is neither: an audit covers the criteria its verdict addresses and no others, so ask the role about them and quote what came back.
