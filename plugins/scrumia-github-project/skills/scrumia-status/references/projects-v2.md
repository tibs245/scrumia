...
`issues` is the one command here that leaves Projects v2 alone, and the distinction is the point rather than an implementation detail: a card leaves the board when its work does, so a board search for something already settled comes back empty and reads as "never raised". It takes no `--state` flag — a search reaching only what is open cannot answer the question it exists for — and its answer carries `surface` and `states` so a board read can never be mistaken for it. It needs `project.repo` and nothing the board contributes, so it still answers on a project whose board ids are unresolved.
