...
2. The board, primary source — read it through the tool, never by composing `gh project` calls yourself:
   ...
   Items labelled `discussion` come back under `discussions`, out of `columns`: an issue holding something unresolved is not work waiting to be started, and counting it as one is how a backlog stops being readable. Report them as a count, in one line, and never in **Ready to go** or **Points of attention** — a discussion with no acceptance criterion and no movement is not a finding. The subtraction is the board read's, so the same one applies here and to the next-step reading (`features/business/github-tracking/business.md` § *Labels and their consumers*); the fallback in 3 is the one path where it is yours to apply.
...
3. Fallback, when `tracker.project_number` is missing from the config, `scrumia-board` errors, or `gh` lacks the `project` scope: `gh issue list --state open --json number,title,labels,assignees,createdAt` and `gh pr list --json number,title,isDraft,reviewDecision,headRefName`. Nothing subtracts `discussion` for you on this path — the board read is what does it — so read the `labels` field and set those issues aside yourself, and say in the report that you did: a subtraction applied by hand is one a reader cannot tell from one that was forgotten. Say explicitly too that it reflects issue/PR state, not the board's actual columns, whenever this path is taken. `scrumia-board doctor` names which of the three is missing.
