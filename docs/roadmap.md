...
- [x] `practices` slot ([ADR-0010](adr/0010-cross-cutting-practices.md)) with `scrumia-practice-tdd` and `scrumia-practice-solid`
- [x] `scrumia-practice-tanstack-query` — the `practices` slot's third answer, TanStack Query for server state; reference skill only, audit and refactor below
...
- [ ] `scrumia-practice-tanstack-query` — audit and refactor skills, closing the loop the reference skill opened
